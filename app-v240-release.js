// v2.5.0 official release loader: stable worksheet practice + print test + focus/reward system.
(() => {
  ...
  function wirePrint(){const old=document.getElementById('weeklyStaticOpenV202');if(!old||old.dataset.v250wired==='1')return;const b=old.cloneNode(true);b.dataset.v204wired='1';b.dataset.v240wired='1';b.dataset.v250wired='1';b.textContent='プリントでテスト';old.replaceWith(b);b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();if(typeof window.openPrintTestV230==='function')window.openPrintTestV230();});}
  ...
  function load(src,done){if(document.querySelector(`script[data-v250="${src}"]`)){done?.();return;}const s=document.createElement('script');s.src=src;s.dataset.v250=src;s.onload=()=>done?.();s.onerror=()=>console.error('Kanji Quest: failed to load',src);document.body.appendChild(s);}
  refresh();
  load('./app-v250-island.js?v=2500',()=>load('./app-v250-study.js?v=2500',()=>{refresh();window.MioriV250?.polishIsland?.();}));
})();
