...
const doc={createElement:node,head:{appendChild(){}},querySelector(){return null},getElementById(){return null}};
...
const code=src('app-v236-school-test.js').replace('  const old=window.openPrintTestV230;', '  window.__schoolTest={record,gradeOne,saveResult,write:a=>answers=a,mode:m=>mode=m};\n  const old=window.openPrintTestV230;');vm.runInContext(code,ctx);
...
(async()=>{
 ...
 api.mode('test');rows.forEach(row=>row.forEach(a=>a.result={pass:true,manual:false}));api.saveResult();
