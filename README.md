...
- 返却された漢字テスト14（9/25・85点）を履歴に記録。実テストのかな文と指定線を、直線は漢字、波線は漢字と送り仮名のtargetとして共有します。
- 「今週の学校テスト」では10問のうち好きな問題を選び、線1本につき一つの欄へ指定部分だけを書きます。途中で答え合わせをすれば練習、10問全部で答え合わせをすれば週別テスト結果を記録します。自分の字と正答は左右に並べて比較します。A4横版の印刷レイアウトは従来どおりです。
