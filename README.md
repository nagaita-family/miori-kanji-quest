...
- 今週の10問テストの「A4プリント・PDF」から、読み付きの縦書き練習プリントを開けます。A4横に①〜⑩を右から左へ並べ、全問を仕切りのない大きな長方形の記入欄にしました。送り仮名を含む読みには赤い波線を付けます。プレビューの「印刷・PDFとして保存」で印刷またはブラウザのPDF保存を選べます。採点前の正答や書いた字は載せません。
- 返却された漢字テスト14（9/25・85点）を履歴に記録。実テストのかな文と指定線を、直線は漢字、波線は漢字と送り仮名のtargetとして10問・印刷・学校テストれんしゅうで共有します。通常の本命漢字練習とは別に取り組めます。
...
- 親向けの練習プリントの書式・配布方法の拡充（今週の10問のA4印刷・PDF保存は対応済み）
- 過去のテスト範囲を使った復習モード
