...
assert.ok(schoolUi.includes('count}か所を1ページで記入'),'Question screen explains that all marked targets are on one page');
assert.ok(schoolUi.includes('次の問題 →'),'Primary navigation advances to the next question');
