...
async function adjacent(p){const [canvas,frame,problem,writing,kana]=await Promise.all([p.locator('#schoolFocusV236 canvas').first().boundingBox(),p.locator('#schoolFocusV236 .schoolCanvasWrapV236').first().boundingBox(),p.locator('.schoolProblemV236').boundingBox(),p.locator('.schoolWriteV236').boundingBox(),p.locator('#schoolFocusV236 .schoolProblemV236 .schoolSentenceV236').boundingBox()]);assert.ok(canvas&&frame&&problem&&writing&&kana&&canvas.x+canvas.width<problem.x&&kana.x-(canvas.x+canvas.width)<35,`Kana text must be next to the answer: ${JSON.stringify({canvas,kana})}`);assert.ok(frame.y>=writing.y&&frame.y+frame.height<=writing.y+writing.height+1,`All four edges of the answer frame are visible: ${JSON.stringify({frame,writing})}`);return canvas}
