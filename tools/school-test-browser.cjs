...
async function adjacent(p){const [canvas,frame,problem,writing]=await Promise.all([p.locator('#schoolFocusV236 canvas').first().boundingBox(),p.locator('#schoolFocusV236 .schoolCanvasWrapV236').first().boundingBox(),p.locator('.schoolProblemV236').boundingBox(),p.locator('.schoolWriteV236').boundingBox()]);assert.ok(canvas&&frame&&problem&&writing&&canvas.x+canvas.width<problem.x&&problem.x-(canvas.x+canvas.width)<20,`Answer must be just left of prompt: ${JSON.stringify({canvas,problem})}`);assert.ok(frame.y>=writing.y&&frame.y+frame.height<=writing.y+writing.height+1,`All four edges of the answer frame are visible: ${JSON.stringify({frame,writing})}`);return canvas}
