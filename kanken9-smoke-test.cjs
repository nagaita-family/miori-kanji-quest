...
const loaderVersion=html.match(/app-v240-release\.js\?v=(\d+)/);
assert.ok(loaderVersion&&Number(loaderVersion[1])>=2860,'iPad must request a cache-busted loader at least as recent as v2.8.6');
assert.ok(html.includes('id="helpPips"')&&html.includes('id="strokeMsg"')&&html.includes('id="okuriPrompt"'),'School practice must retain all legacy DOM hooks');
...
console.log('PASS: 240 entries, cache-busted loader, mock 20, compact writing and review, school hooks and separate module safety.');
